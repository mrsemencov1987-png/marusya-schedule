...
def get_current_day():
    ...
    # В русском календаре понедельник - 0, воскресенье - 6
...
def get_next_monday():
    ...
    if days_ahead <= 0:  # Если сегодня не понедельник или понедельник уже прошёл
        days_ahead += 7
...
def reset_changes_if_monday():
    ...
    today = datetime.date.today()
    last_monday = get_next_monday() - datetime.timedelta(days=7)
    
    # Проверяем, нужно ли сбросить
    if today.weekday() == 0:  # Понедельник
        # Проверяем, сбрасывали ли уже сегодня
...
def apply_changes(day, lessons):
    ...
    if day_lower in changes:
        for change in changes[day_lower]:
            # Проверяем формат изменения
            parts = change.split(":")
            if len(parts) >= 2:
                try:
                    index = int(parts[0]) - 1  # 1-based -> 0-based
...
def parse_change_command(command):
    ...
    # Пример: "замени математику на физику во вторник"
    ...
    if "замени" in command and " на " in command:
        # Пытаемся вытащить старый и новый урок
        # Упрощённо: берём всё между "замени" и "на"
        parts = command.split("замени")[1].split(" на ")
        if len(parts) >= 2:
            old_lesson = parts[0].strip()
            # Отрезаем название дня, если оно есть в конце
...
def webhook():
    ...
    # Сбрасываем изменения, если понедельник
    reset_changes_if_monday()
    
    # Данные от пользователя
    command = req.get("request", {}).get("command", "").lower()
    original = req.get("request", {}).get("original_utterance", "")
    ...
    # Определяем, какой день спрашивают
    day_to_show = None
